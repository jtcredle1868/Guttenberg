...
import { salesRecords, formats, titles, disbursements } from '../data/mockData';
...
function calcRoyalty(
  ...
  printingCost: number,
  fileSizeMB = 2
): { royalty: number; rate: string; formula: string } {
  switch (channel) {
    case 'amazon_kdp': {
      if (formatType === 'ebook') {
...
router.get('/titles/:id/royalties', (req: Request, res: Response) => {
  ...
  const breakdown = titleFormats.flatMap((fmt) =>
    title.distributionChannels.map((ch) => {
      const calc = calcRoyalty(ch, fmt.type, fmt.price, fmt.printingCost);
      const sales = salesRecords.filter((s) => s.formatId === fmt.id && s.channel === ch);
...
router.get('/earnings', (req: Request, res: Response) => {
  ...
  if (startDate) filtered = filtered.filter((s) => new Date(s.date) >= startDate);
  if (endDate) filtered = filtered.filter((s) => new Date(s.date) <= endDate);
  ...
  // By month
  const monthMap: Record<string, { month: string; revenue: number; royalties: number; units: number }> = {};
  for (const s of filtered) {
    const month = s.date.slice(0, 7);
    if (!monthMap[month]) monthMap[month] = { month, revenue: 0, royalties: 0, units: 0 };
  ...
  // By channel
  const channelMap: Record<string, { channel: string; revenue: number; royalties: number; units: number }> = {};
  for (const s of filtered) {
    if (!channelMap[s.channel]) channelMap[s.channel] = { channel: s.channel, revenue: 0, royalties: 0, units: 0 };
...
router.get('/sales', (req: Request, res: Response) => {
  const { titleId, format: formatType, channel, startDate, endDate } = req.query as Record<string, string>;
  const page = Math.max(1, parseInt(req.query.page as string) || 1);
  const limit = Math.min(200, Math.max(1, parseInt(req.query.limit as string) || 50));

  let filtered = [...salesRecords];
  if (titleId) filtered = filtered.filter((s) => s.titleId === titleId);
  ...
  if (formatType) {
    const matchingFormats = formats.filter((f) => f.type === formatType).map((f) => f.id);
...
router.get('/disbursements', (req: Request, res: Response) => {
  return res.json({ disbursements });
});

// POST /api/finance/royalty-calculator
router.post('/royalty-calculator', (req: Request, res: Response) => {
  ...
  const targetChannels: string[] = channels.length > 0
    ? channels
