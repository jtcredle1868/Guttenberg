...
import { titles, formats, distributionRecords, versionHistory } from '../data/mockData';
...
router.get('/:id', (req: Request, res: Response) => {
  ...
  const titleDist = distributionRecords.filter((d) => d.titleId === req.params.id);
...
router.put('/:id', (req: Request, res: Response) => {
  const idx = titles.findIndex((t) => t.id === req.params.id);
  if (idx === -1) {
    return res.status(404).json({
      error: {
        code: 'GUT-4041',
  ...
  titles[idx] = { ...titles[idx], ...updates, updatedAt: new Date().toISOString() };
  return res.json(titles[idx]);
});
...
router.get('/:id/publishing-readiness', (req: Request, res: Response) => {
  const title = titles.find((t) => t.id === req.params.id);
  if (!title) {
    return res.status(404).json({
      error: {
  ...
  const overall = Math.round(
    refineryScore * 0.6 + metadataCompleteness * 0.2 + preflightScore * 0.2
  );
...
function getMissingMetadataFields(title: any): string[] {
  const missing: string[] = [];
  if (!title.description || title.description.length < 50) missing.push('description');
  if (!title.genre) missing.push('genre');
  if (!title.bisacCodes || title.bisacCodes.length === 0) missing.push('bisacCodes');
  if (!title.keywords || title.keywords.length === 0) missing.push('keywords');
  if (!title.language) missing.push('language');
  if (!title.publicationDate) missing.push('publicationDate');
  return missing;
}
...
router.get('/:id/versions', (req: Request, res: Response) => {
  ...
  const versions = versionHistory[titleId] || [];
...
router.post('/:id/versions', (req: Request, res: Response) => {
  ...
  const titleId = req.params.id as string;
  if (!versionHistory[titleId]) versionHistory[titleId] = [];
  const existing = versionHistory[titleId];
