...
router.get('/titles/:id/distribution', (req, res) => {
    ...
    const records = mockData_1.distributionRecords.filter((d) => d.titleId === req.params.id);
...
router.post('/titles/:id/distribution', (req, res) => {
    const title = mockData_1.titles.find((t) => t.id === req.params.id);
    if (!title) {
        return res.status(404).json({
            error: { code: 'GUT-4041', message: 'Title not found', detail: `No title with id '${req.params.id}'.`, resolution: 'Verify the title ID.', docs_url: 'https://docs.guttenberg.io/errors/GUT-4041' },
    ...
    for (const channel of channels) {
        const existing = mockData_1.distributionRecords.find((d) => d.titleId === req.params.id && d.channel === channel);
        if (existing)
            continue;
        const newRecord = {
            id: `dist-${(0, uuid_1.v4)().slice(0, 8)}`,
        ...
        mockData_1.distributionRecords.push({ ...newRecord, titleId: newRecord.titleId });
        submitted.push(newRecord);
        const job = { jobId: `distjob-${(0, uuid_1.v4)().slice(0, 8)}`, channel, status: 'queued', estimatedDays: 3 };
        jobs.push(job);
    }
    return res.status(202).json({ titleId: req.params.id, submitted, jobs });
});
// DELETE /api/titles/:id/distribution/:channelId
router.delete('/titles/:id/distribution/:channelId', (req, res) => {
    const idx = mockData_1.distributionRecords.findIndex((d) => d.titleId === req.params.id && d.id === req.params.channelId);
    if (idx === -1) {
        return res.status(404).json({
    ...
    mockData_1.distributionRecords.splice(idx, 1);
    return res.status(204).send();
});
// GET /api/titles/:id/channel-readiness
router.get('/titles/:id/channel-readiness', (req, res) => {
    const title = mockData_1.titles.find((t) => t.id === req.params.id);
    if (!title) {
        return res.status(404).json({
            error: { code: 'GUT-4041', message: 'Title not found', detail: `No title with id '${req.params.id}'.`, resolution: 'Verify the title ID.', docs_url: 'https://docs.guttenberg.io/errors/GUT-4041' },
