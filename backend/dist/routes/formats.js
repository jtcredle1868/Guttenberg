...
router.get('/titles/:id/formats', (req, res) => {
...
router.post('/titles/:id/formats', (req, res) => {
    const title = mockData_1.titles.find((t) => t.id === req.params.id);
    if (!title) {
        return res.status(404).json({
            error: { code: 'GUT-4041', message: 'Title not found', detail: `No title with id '${req.params.id}'.`, resolution: 'Verify the title ID.', docs_url: 'https://docs.guttenberg.io/errors/GUT-4041' },
    ...
    const jobId = `job-${(0, uuid_1.v4)().slice(0, 8)}`;
    const newJob = {
        id: jobId,
    ...
    mockData_1.formatJobs.push({ ...newJob, titleId: newJob.titleId });
...
router.get('/:jobId/status', (req, res) => {
    const job = mockData_1.formatJobs.find((j) => j.id === req.params.jobId);
    if (!job) {
        return res.status(404).json({
